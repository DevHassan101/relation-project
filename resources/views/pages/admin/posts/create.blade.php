<x-app-layout>
    ...
    <div class=" bg-white border border-gray-300 rounded-2xl">
        <form class="bg-white rounded px-3 pt-6 pb-4 mb-4" action="{{ route('posts.store') }}" method="post"
            ...
            <div class="grid sm:grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div class="">
                    ...
                    <input class="border rounded w-full py-2 px-3 text-gray-700" id="title" name="title"
                        value="{{ old('title') }}" type="text" placeholder="Enter title">
                    @error('title')
            ...
            <div class="">
                ...
                <textarea id="content" name="content" rows="6" placeholder="Write content here"
                    class="w-full p-2 border rounded-md text-black dark:border-gray-500">{{ old('content') }}</textarea>
                @error('title')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
